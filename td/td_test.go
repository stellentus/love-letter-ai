...
func TestFileLoadSave(t *testing.T) {
	...
	tableSize := 100
	td := newTestTDlayer(epsilon, alpha, gamma, tableSize)
	...
	td2 := newTestTDlayer(epsilon, alpha, gamma, tableSize)
	err = td2.LoadFromFile(path)
	assert.NoError(t, err)

	assert.Equal(t, epsilon, td2.Epsilon, "Epsilon didn't save/load the same")
...
func newTestTDlayer(epsilon, alpha, gamma float32, size int) *TD {
	return &TD{
